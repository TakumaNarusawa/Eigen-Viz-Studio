...
ブラウザ内で動作する強力な数学エンジン（**Pyodide & SymPy**）と、美しい 3D WebGL グラフィックス（**Three.js**）を融合させ、空間の変形と固有空間の関係性を、直感的かつ精緻なビジュアルで表現します。
...
- $\mathbf{A} = \mathbf{P}\mathbf{D}\mathbf{P}^{-1}$ のプロセスを独自の幾何学アニメーションで視覚的に説明。
  - **Step 1 (基底変換 $\mathbf{P}^{-1}$)**: 固有ベクトルを新しい座標軸とする「固有空間」へ世界を歪ませます。
  - **Step 2 (伸縮 $\mathbf{D}$)**: 新しい座標軸に沿って、固有値倍だけ純粋に直交伸縮させます。
  - **Step 3 (元の空間への射影 $\mathbf{P}$)**: 伸縮された世界を、元の座標系へと引き戻します。

### 4. 数式とグラフィックスのホバー連動インタラクション
- **ダイナミック・ネオンハイライト**: 解析結果パネルに表示される数式（KaTeX）にマウスホバーすると、2D Canvas / 3D WebGL上の対応する固有空間の軸（または2Dの複素不変楕円の軌道）が自動的に強くネオン発光し、その他の非アクティブな軸は薄暗くフェードアウトします。数式と幾何学的意味を一瞬で結びつける極上のUI/UXインタラクションです。

### 5. 高解像度ディスプレイ（DPI/Retina）対応
- **超シャープなグラフィックス**: `devicePixelRatio` に基づいて Canvas の物理ピクセル解像度を自動調整し、高画素密度環境下であってもベクトルやグリッド線、テキストが一切ぼやけることなく、極めて鮮明で美しく描画されます。
